Altair Core version 3.0.2 is now available from:
...
Altair Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zATR spends, automint calculation adjustments, and other various updates/fixes.
...
Users that wish to prevent this behavior (not recommended) can pass `-backupzATR=0` at the command-line when starting the client, or add `backupzATR=0` to their `Altair.conf` file.
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the Altair address as it's first parameter and the passphrase as it's second parameter.
