...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Altair address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no Altair: URI
...
{
    // Convert Altair:// to Altair:
    //
    //    Cannot handle this later, because Altair:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open Altair.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Altair.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Altair.lnk
...
{
    return GetAutostartDir() / "Altair.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a Altair.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Altair app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Altair app to startup item list
