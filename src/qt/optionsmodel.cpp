...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAltairAmount"))
        settings.setValue("nAnonymizeAltairAmount", 1000);

    nAnonymizeAltairAmount = settings.value("nAnonymizeAltairAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAltairAmount"))
        SoftSetArg("-anonymizeAltairamount", settings.value("nAnonymizeAltairAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in Altair.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAltairAmount:
            return QVariant(nAnonymizeAltairAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAltairAmount:
            nAnonymizeAltairAmount = value.toInt();
            settings.setValue("nAnonymizeAltairAmount", nAnonymizeAltairAmount);
            emit anonymizeAltairAmountChanged(nAnonymizeAltairAmount);
            break;
        case CoinControlFeatures:
