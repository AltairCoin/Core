...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    nBlockMaxSize = std::max((unsigned int)1000, std::min((unsigned int)(500000 - 1000), nBlockMaxSize));
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AltairMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AltairMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AltairMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Altair-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < 1515782262 || vNodes.empty() || pwallet->IsLocked() || !pwallet->MintableCoins() || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
        ...
        LogPrintf("Running AltairMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
