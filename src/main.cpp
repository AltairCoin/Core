...
#if defined(NDEBUG)
#error "Altair cannot be compiled without assertions."
#endif
        ...
        if(tx.IsZerocoinSpend()){
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zATR spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zATR spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 0; // Block confirm period
    } else if (nHeight > 5000 && nHeight <= 64800) {
        nSubsidy = 105 * COIN;  
    } else if (nHeight > 64800 && nHeight <= 129600) {
        nSubsidy = 99 * COIN;
    } else if (nHeight > 129600 && nHeight <= 194400) {
        nSubsidy = 96 * COIN;
    } else if (nHeight > 194400 && nHeight <= 324000) {
        nSubsidy = 99 * COIN;
    } else if (nHeight > 324000 && nHeight <= 388800) {
        nSubsidy = 101 * COIN;
    }  else if (nHeight > 388800 && nHeight <= 453600) {
        nSubsidy = 95 * COIN;
    }  else if (nHeight > 453600 && nHeight <= 518400) {
        nSubsidy = 93 * COIN;
    }  else if (nHeight > 518400 && nHeight <= 583200) {
        nSubsidy = 95 * COIN;
    }  else if (nHeight > 583200 && nHeight <= 648000) {
        nSubsidy = 97 * COIN;
    }  else if (nHeight > 648000 && nHeight <= 712800) {
        nSubsidy = 91 * COIN;
    }  else if (nHeight > 712800 && nHeight <= 777600) {
        nSubsidy = 90 * COIN;
    }  else if (nHeight > 777600 && nHeight <= 842400) {
        nSubsidy = 91 * COIN;
    }  else if (nHeight > 842400 && nHeight <= 907200) {
        nSubsidy = 93 * COIN;
    }  else if (nHeight > 907200 && nHeight <= 972000) {
        nSubsidy = 89 * COIN;
    }  else if (nHeight > 972000 && nHeight <= 1036800) {
        nSubsidy = 88 * COIN;
    }  else if (nHeight > 1036800 && nHeight <= 1101600) {
        nSubsidy = 89 * COIN;
    }  else if (nHeight > 1101600 && nHeight <= 1166400) {
        nSubsidy = 87 * COIN;
    }  else {
        nSubsidy = max((60.55 - (0.5 * ((1166400 - nHeight) / 64800))) * COIN, 0.001);//Smooth subsidy lowering, -0.5 every 64800 blocks
    }
    return nSubsidy;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 0; // Block confirm period
    } else if (nHeight > 999 && nHeight <= 64800) {
        nSubsidy = 31.5 * COIN;  
    } else if (nHeight > 64800 && nHeight <= 129600) {
        nSubsidy = 29.7 * COIN;
    } else if (nHeight > 129600 && nHeight <= 194400) {
        ...
        nSubsidy = max((25.95 - (0.5 * ((1166400 - nHeight) / 64800))) * COIN, 0.001);//Smooth subsidy lowering, -0.5 every 64800 blocks
    }
    return nSubsidy;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Altair
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zATR with serial %s is already in the block %d\n",
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Altair: We use certain sporks during IBD, so check to see if they are
